...
def paint_status(win_x, win_y, win_width, win_height, status_buf):
    movec(0, win_y)
    print(status_buf[:win_width], end = "")

def paint_cmd(win_x, win_y, win_width, win_height, cmd_buf, cx):
    movec(0, win_y)
...
try:
    while 1:
        if edit_mode == "normal" or edit_mode == "insert":
            #buf = edit_buf[0][1]
            buf_filename, buf, undo_buf = edit_buf[0]
        ...
        status_buf = "{} {:d},{:d}".format(buf_filename, cx + 1, gy + cy + 1)

        clear()
        paint_status(0, win_height - 1, edit_width, edit_height, status_buf)
